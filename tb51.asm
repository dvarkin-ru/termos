...
;
EXTRAM		EQU	0134H		;External program buffer begins after 26 vars.
RAMLIM		EQU	0400H		;Allowance made for 1K RAM buffer.
EXTROM		EQU	2080H		;Start of external ROM space.
...
MFLAGS:		DS	1		;Machine flags (typewriter)
F1		BIT	MFLAGS.0	;FIRE !!!
TOSTART		BIT	MFLAGS.1	;Carriage to start
KEYPR		BIT	MFLAGS.2	;Keyboard key pressed
KEYRDD		BIT	MFLAGS.3	;Keyboard key readed by BASIC
INK		BIT	MFLAGS.4	;INK RIBBON FLOW

UART_F:		DS	1		;UART flags
DBGSER		BIT	UART_F.0	;Basic command line t. UART
FLOW		BIT	UART_F.1	;Flow control bit
FLWTGL		BIT	UART_F.2	;Flow state changed
XOFF		BIT	UART_F.3	;XON/XOFF state

LFST:		DS	1		;NUM OF HALF-ROTATIONS
...
;
	CSEG
	ORG	0000H
	JMP	SP_INI		;Jump to system initialization routine.
...
RD_KEY:	MOV	A,KEYBUF
	CLR	KEYRDD
	JMP	C_IN_0
RD_SER:	MOV     A,SBUF          ;Read input character.
	CLR		RI				;Clear reception flag.
C_IN_0:	ANL		A,#7FH			;Mask off data bits.
...
DIV_2:	CALL	NEG_IF_NEG
	RET
;
DIV_NG:	JMP	EXP_OV		;Report expression overflow.
;
;===============
;
;$EJECT
...
RAM_INIT:
	...
	MOV	R0,A
	MOV	DPTR,#(EXTRAM - (2*26))
RAM_1:	MOVX	@DPTR,A
	INC	DPTR
	INC	R0
	CJNE	R0,#2*26,RAM_1	;Loop until all vars cleared.
	POP	DPH
	POP	DPL
	RET
;
...
s19b:   call   tst
        ...
        ICALL_  EXPR
        CALL    DONE
        CALL    BORDER		; Setup right border.
        JMP     NXT
;
...
s19e:   call   tst
        ...
        CALL    DONE
	CALL	POP_TOS
	MOV	BRT,TOS_L	; Setup baudrate timer
        JMP     NXT
...
s19g:   call   tst
        db      'DB',('G' OR 80H)	; Attach interpreter to UART
        jnc     s19h
        ICALL_  EXPR
        CALL    DONE
        CALL	POP_TOS
...
;
;Parse GETCH command.
;Copypasted from INPUT and changed
s19h:     call   tst
        db      'GETC',('H' OR 80H)
        jnc     s193
S10h:    TSTS_   S10HB
...
;
;Parse PUTCH command.
;Copypasted from PRINT and changed
s193:     call   tst
        db      'PUTC',('H' OR 80H)
        jnc     s20
        IFDONE_ S196A
...
TSTSS:	CALL	READ_CHAR
	...
	RET
;
TSTSS1:	CALL	READ_CHAR	;Read next string character.
	CJNE	A,TMP0,TSTSS2
	JMP	D_BLNK
;
TSTSS2:	JNB	TI, $
	CLR	TI
        MOV	SBUF, A
	CJNE	A,#CR,TSTSS1	;<CR> before closing quote is illegal.
	JMP	SYN_ER		;Transmit error message.
